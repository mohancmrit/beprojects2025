...
# Initialize round-robin index for each room
room_indices = [0] * len(rooms)
...
for input_slot in slot_list:
    ...
    for detail in details:
        if detail.get("slot") == input_slot:
            code_list.append(detail.get("branch"))
    print(code_list)
    s = details[0].get("sem")
    ...
    for code in code_list:
        ...
        # CHECKING IF THERE ARE SUPPLY STUDENTS OR NOT
        try:
